...
$schoolQuery = 'SELECT * FROM `__global`.`gpr_schools` WHERE id="3"';
...
while($schoolRow = mysql_fetch_array($schoolResult)){
	...
	if(strtotime($schoolRow['reg_close']) <= strtotime($today)){ 
		...
		if(strtotime($schoolRow['reg_open']) >= strtotime($today)){ 
			...
			echo " at 9:00am Central Time.</span>";			
		}
		else {
			...
			<A href='http://www.atkearney.com/careers/global-prize-registration-kellogg-school-of-management/register' target='_parent'>Click here to register</a>.</span>";	
