...
$schoolQuery = 'SELECT * FROM `__global`.`gpr_schools` WHERE id="7"';
...
while($schoolRow = mysql_fetch_array($schoolResult)){
	...
	if(strtotime($schoolRow['reg_close']) <= strtotime($today)){ 
		...
		if(strtotime($schoolRow['reg_open']) >= strtotime($today)){ 
			...
			<A href='http://www.atkearney.com/careers/global-prize-registration-cranfield-school-of-management/register' target='_parent'>Click here to register</a></div>";	
