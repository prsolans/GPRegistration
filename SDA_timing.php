...
$schoolQuery = 'SELECT * FROM `__global`.`gpr_schools` WHERE id="11"';
...
while($schoolRow = mysql_fetch_array($schoolResult)){
	...
	if(strtotime($schoolRow['reg_close']) <= strtotime($today)){ 
		...
		if(strtotime($schoolRow['reg_open']) >= strtotime($today)){ 
			...
			echo "<span style='font: 12px/1.5 Arial, Helvetica, Verdana, sans-serif; padding: 0; margin: 0;'>Registration for Global Prize 2012 is now open.<A href='http://www.atkearney.com/careers/global-prize-registration-sda-bocconi/register' target='_parent'>Click here to register</a></div>";	
